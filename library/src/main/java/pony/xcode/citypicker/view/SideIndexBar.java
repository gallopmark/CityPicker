package pony.xcode.citypicker.view;
...
public class SideIndexBar extends View {
    ...
    private void init(Context context) {
        ...
        int textTouchedColor = context.getResources().getColor(typedValue.resourceId);
        ...
        mTouchedPaint.setTextSize(textSize);
        mTouchedPaint.setColor(textTouchedColor);
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (Math.abs(h - oldh) == navigationBarHeight) {
            ...
            //避免软键盘弹出时挤压
            height = Math.max(getHeight(), oldh);
        }
        mItemHeight = height / ((float) mIndexItems.size());
        mTopMargin = (height - mItemHeight * mIndexItems.size()) / 2;
    }
